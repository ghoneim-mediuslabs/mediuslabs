...
import { Globe, Users, School, Truck, Building2, ArrowRight, ArrowLeft, CheckCircle2, Sparkles, XCircle, Banknote, MessageSquare, BarChart3, Shield, Smartphone, Zap, HeartHandshake, Phone, Mail, MapPin, CreditCard } from 'lucide-react'
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="px-4 py-16 bg-gray-50">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            <h2 className="text-3xl font-bold text-gray-900 mb-4">
              {isAr ? 'من يرى ماذا؟' : 'Who Sees What?'}
            </h2>
            <p className="text-gray-600 max-w-2xl mx-auto">
              {isAr
                ? 'كل مدرسة لها بيئة منفصلة تماماً — لا يمكن لأي طرف الوصول لبيانات مدرسة أخرى'
                : 'Each school has a completely separate environment — no party can access another school\'s data'}
            </p>
          </div>

          <div className="max-w-3xl mx-auto">
            <div className="rounded-3xl border-2 border-violet-300 bg-violet-50/50 p-3 sm:p-6">
              <div className="flex items-center gap-2 mb-4">
                <Building2 size={20} className="text-violet-600" />
                <span className="font-semibold text-violet-700">
                  {isAr ? 'مجموعة المدارس' : 'School Group'}
                </span>
                <span className="text-xs bg-violet-100 text-violet-600 px-2 py-0.5 rounded-full">
                  {isAr ? 'ترى كل فروعها' : 'sees all its branches'}
                </span>
              </div>

              <div className="space-y-4">
                <div className="rounded-2xl border-2 border-emerald-300 bg-emerald-50/50 p-3 sm:p-4">
                  <div className="flex items-center gap-2 mb-3">
                    <School size={18} className="text-emerald-600" />
                    <span className="font-semibold text-emerald-700">
                      {isAr ? 'المدرسة أ' : 'School A'}
                    </span>
                    <span className="text-xs bg-emerald-100 text-emerald-600 px-2 py-0.5 rounded-full">
                      {isAr ? 'ترى طلابها فقط' : 'sees its students only'}
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <div className="rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 flex items-center gap-2">
                      <Users size={14} className="text-blue-600" />
                      <span className="text-sm text-blue-700">{isAr ? 'ولي أمر ١' : 'Parent 1'}</span>
                    </div>
                    <div className="rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 flex items-center gap-2">
                      <Users size={14} className="text-blue-600" />
                      <span className="text-sm text-blue-700">{isAr ? 'ولي أمر ٢' : 'Parent 2'}</span>
                    </div>
                    <div className="rounded-xl border border-orange-200 bg-orange-50 px-3 py-2 flex items-center gap-2">
                      <Truck size={14} className="text-orange-600" />
                      <span className="text-sm text-orange-700">{isAr ? 'مورد الزي' : 'Uniform Co.'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-2xl border-2 border-emerald-300 bg-emerald-50/50 p-3 sm:p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <div className="rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 flex items-center gap-2">
                      <Users size={14} className="text-blue-600" />
                      <span className="text-sm text-blue-700">{isAr ? 'ولي أمر ٣' : 'Parent 3'}</span>
                    </div>
                    <div className="rounded-xl border border-orange-200 bg-orange-50 px-3 py-2 flex items-center gap-2">
                      <Truck size={14} className="text-orange-600" />
                      <span className="text-sm text-orange-700">{isAr ? 'مورد الكتب' : 'Book Supplier'}</span>
                    </div>
                  </div>
                </div>
              </div>

              <div className="mt-4 flex items-start gap-2 bg-white rounded-xl p-3 border border-gray-200">
                <Shield size={18} className="text-gray-600 flex-shrink-0 mt-0.5" />
                <p className="text-sm text-gray-600">
                  {isAr
                    ? 'لا يمكن للمدرسة ب الوصول لبيانات المدرسة أ والعكس صحيح — فصل كامل للبيانات'
                    : 'School B cannot access School A\'s data and vice versa — complete data isolation'}
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="px-4 py-16">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            <h2 className="text-3xl font-bold text-gray-900 mb-4">
              {isAr ? 'كيف تتم المدفوعات' : 'How Payments Work'}
            </h2>
            <p className="text-gray-600 max-w-2xl mx-auto">
              {isAr
                ? 'جميع المدفوعات تمر عبر المنصة بشكل آمن'
                : 'All payments flow securely through the platform'}
            </p>
          </div>
          ...
          <div className="space-y-6 max-w-2xl mx-auto">
            {[
              {
                titleAr: 'دفع الرسوم', titleEn: 'Fee Payment',
                steps: [
                  { icon: Users, labelAr: 'ولي الأمر يدفع', labelEn: 'Parent pays', bg: 'bg-blue-100', text: 'text-blue-600' },
                  { icon: CreditCard, labelAr: 'عبر المنصة', labelEn: 'Via platform', bg: 'bg-gray-100', text: 'text-gray-600' },
                  { icon: School, labelAr: 'المدرسة تستلم', labelEn: 'School receives', bg: 'bg-emerald-100', text: 'text-emerald-600' },
                ],
              },
              {
                titleAr: 'شراء المنتجات', titleEn: 'Product Purchase',
                steps: [
                  { icon: Users, labelAr: 'ولي الأمر يطلب', labelEn: 'Parent orders', bg: 'bg-blue-100', text: 'text-blue-600' },
                  { icon: CreditCard, labelAr: 'عبر المنصة', labelEn: 'Via platform', bg: 'bg-gray-100', text: 'text-gray-600' },
                  { icon: Truck, labelAr: 'المورد يوفر', labelEn: 'Supplier fulfills', bg: 'bg-orange-100', text: 'text-orange-600' },
                ],
              },
            ].map((flow, fi) => (
              <div key={fi} className="bg-white rounded-2xl border border-gray-200 p-6">
                    ...
                    return (
                      <div key={i} className="flex items-center gap-3 sm:gap-5">
                        <div className="flex flex-col items-center gap-1.5">
                          <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-full ${step.bg} flex items-center justify-center`}>
                            <StepIcon size={22} className={step.text} />
                          </div>
                        ...
                        {i < flow.steps.length - 1 && (
                          <Arrow size={20} className="text-gray-300 flex-shrink-0 -mt-5" />
                        )}
                      </div>
                    )
                  })}
            ...
            <div className="bg-violet-50 border border-violet-200 rounded-xl p-4 flex items-start gap-3">
              <Building2 size={22} className="text-violet-600 flex-shrink-0 mt-0.5" />
              <p className="text-sm text-violet-700">
                {isAr
                  ? 'مجموعة المدارس ترى إجمالي المدفوعات والإيرادات لجميع فروعها في لوحة تحكم موحدة'
                  : 'School Group sees total payments and revenue across all branches in a unified dashboard'}
              </p>
            </div>
          </div>
        </div>
